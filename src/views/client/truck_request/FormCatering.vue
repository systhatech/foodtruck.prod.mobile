            ...
            <v-row>
                <v-col cols="12" v-if="step==1">
                 ...
                 <v-col cols="12" v-if="step==3">
                    <div class="custom-bs pa-4 w-100 text-center pt-6">
                        <label for="" class="text-center mb-6" :class="errors.includes('people_minimum') ?'error--text':'primary--text'">Minimum Budget ?</label>
                        <v-text-field class="mt-6" min="1" type="number" :rules="rulesRequiredNumber" v-model="catering.people_minimum"></v-text-field>
                    </div>
                ...
                <v-col cols="12" v-if="step==7">
                 ...
                 <v-col cols="12" v-if="step==8">
                    <div class="custom-bs w-100 text-centerm-bs pa-4 pt-6"> 
                        <div class="text-center">
                            <label for="" :class="errors.includes('selected_cuisine') ?'error--text':'primary--text'">Preferred Cuisine Type</label>
                            <div class="mt-6 pl-12">
                                <v-checkbox v-for="(cuisine, index) in cuisine_types" :key="index"
                                v-model="catering.selected_cuisine"
                                class="pt-0 mt-1"
                                :rules="validateCheckbox"
                                :value="cuisine.value"
                                >
                                    <template v-slot:label>
                                        <div class="">{{  cuisine.name  }}</div>
                                    </template>
                                </v-checkbox>
                            </div>
                        </div>
                 ...
                 <v-col cols="12">
                    <div class="d-flex align-center" :class="step==1?'justify-space-around':'justify-space-between'">
                        ...
                        <v-btn color="primary" v-if="step==8" text large rounded @click="handleSubmit()">submit</v-btn>
...
export default {
    ...
    computed: {
        ...
        validateCheckbox () {
            return [this.catering.selected_cuisine.length > 0 || "Select at least one!"]
        },
        ruleEventType () {
        ...
        handleNext(){
            switch(this.step){
                ...
                case 2:
                    ...
                    break;
                case 3:
                    if(!this.catering.people_minimum){
                        this.errors.push('people_minimum');
                    }else{
                        this.errors = this.errors.filter((item) => item !=='people_minimum');
                        this.step+=1;
                    }
                    break;
                case 4:
                ...
                case 7:
                    ...
                    break;
                case 8:
                    if(!this.catering.selected_cuisine.length){
                        this.errors.push('selected_cuisine');
                    }else{
                        this.errors = this.errors.filter((item) => item !=='selected_cuisine');
                        console.log('submit');
                    }
                    break;
                default:
